...
class Renderer(object):
    ...
    def __init__(self, screen):
        ...
        # Environment map for glEnvMapColor
        self.envMap = None
        
        # Background state
        self.hasBackground = False

        # Env map flip controls
        self.envMapFlipH = False  # flip horizontal (mirror)
        self.envMapFlipV = False  # flip vertical (top/bottom)
    ...
    def glRenderRaytracing(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                # Nota: glPoint escribe con el eje Y invertido (self.height - y - 1),
                # así que generamos py en el mismo sentido para evitar la imagen al revés.
                # y = 0 (parte inferior) -> py = -1 ; y = height-1 (parte superior) -> py = +1
                ...
                color = [0, 0, 0]
                if hit is not None:
                    if hasattr(hit.obj, 'material'):
                        color = hit.obj.material.GetSurfaceColor(hit, self)
                else:
                    if self.hasBackground:
                        # Usar el color del fondo ya dibujado
                        bg = self.frameBuffer[x][y]
                        color = [bg[0] / 255.0, bg[1] / 255.0, bg[2] / 255.0]
                    elif hasattr(self, 'glEnvMapColor'):
                        color = self.glEnvMapColor(self.camera.translation, ray_direction)
    ...
    def glCastRay(self, origin, direction, sceneObj=None, recursion=0):
        if not hasattr(self, 'maxRecursionDepth'):
            self.maxRecursionDepth = 3  # Default value if not set elsewhere
    ...
    def glEnvMapColor(self, orig, dir):
        if self.envMap:
            ...
            if getattr(self, 'envMapFlipH', False):
                x = 1.0 - x
            if getattr(self, 'envMapFlipV', False):
                y = 1.0 - y
            return self.envMap.getColor(x, y)
        # Si no hay envMap, regresa el color de fondo
        if hasattr(self, 'ClearColor'):
            return self.ClearColor
        return [0.7, 0.6, 0.9]  # lila por defecto
